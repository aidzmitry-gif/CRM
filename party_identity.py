...
async def prepare_party_change(session, data, *, deal=None):
    ...
    if deal is not None:
        ...
        if binding is not None and cp_id != binding.counterparty_id:
            raise HTTPException(409, "Выбранная сторона расходится с подтверждённым покупателем; требуется сверка")
